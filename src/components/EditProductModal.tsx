...
export const EditProductModal: React.FC<EditProductModalProps> = ({ product, onClose, addToast }) => {
  ...
  const [regularPrice, setRegularPrice] = useState(product.originalPrice ? String(product.originalPrice) : String(product.price));
  const [discountPercent, setDiscountPercent] = useState(
    product.originalPrice && product.originalPrice > product.price
      ? String(Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100))
      : ""
  );
  ...
  useEffect(() => {
    ...
    // Dynamic Separation of Checklist Selectable Standard sizes vs Custom types
    const standardPresets = ["S", "M", "L", "XL", "XXL", "XXXL"];
    const allSizes = product.sizes || [];
    const standards = allSizes.filter(s => standardPresets.includes(s));
    const customs = allSizes.filter(s => !standardPresets.includes(s));

    setSelectedSizes(standards);
    setCustomSizesInput(customs.join(", "));
    setRegularPrice(product.originalPrice ? String(product.originalPrice) : String(product.price));
    setDiscountPercent(
      product.originalPrice && product.originalPrice > product.price
        ? String(Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100))
        : ""
  ...
  const handleSaveChanges = async (e: React.FormEvent) => {
    ...
    if (!regularPrice || regPriceNum <= 0) return addToast("error", "Please input a valid price");
      ...
      >
        ...
        <form onSubmit={handleSaveChanges} className="flex-1 overflow-y-auto p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* Regular Price / MRP / আসল মূল্য */}
            <div className="space-y-1.5">
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Regular Price / আসল মূল্য (৳) *</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 rounded-xl pl-8 pr-3.5 py-2.5 text-xs outline-none transition bg-white font-mono font-bold text-slate-800"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Discount Percent / ছাড় (%)</label>
                <span className="text-[9px] text-slate-400 font-bold lowercase italic font-sans">Optional</span>
              </div>
              <div className="relative">
                <input
                  type="text"
                  ...
                  placeholder="যেমন: ১৫"
                  className="w-full border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 rounded-xl pl-3.5 pr-8 py-2.5 text-xs outline-none transition bg-white font-mono font-bold text-rose-500"
                />
          ...
          {(category.includes("Fashion") || category.includes("Clothing") || category.includes("Dress")) && (
            <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-4 space-y-3">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Dress Sizes / পোশাকের সাইজ (Standard)</label>
              ...
              <div className="pt-1.5 border-t border-slate-100">
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.55">Custom Sizes / কাস্টম সাইজ (কমা দিয়ে লিখুন)</label>
                <input
                  type="text"
