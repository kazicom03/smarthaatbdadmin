  ...
  return (
    ...
    >
      <div className="space-y-3">
        <div className="aspect-square relative overflow-hidden bg-slate-50 border border-slate-100 rounded-xl p-3 flex flex-col items-center justify-center">
          <img
          ...
          <div className="absolute top-2.5 right-2.5 flex flex-col items-end gap-1.5 z-10">
            <div className="bg-slate-900/90 backdrop-blur-xs text-white text-[11px] font-bold px-2.5 py-1 rounded-lg font-mono shadow-xs flex items-center gap-1.5">
            ...
            {p.originalPrice && p.originalPrice > p.price && (
              <span className="bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md shadow-xs select-none animate-pulse">
                ⚡ {Math.round(((p.originalPrice - p.price) / p.originalPrice) * 100)}% ছাড়
              </span>
            )}
          </div>
...
export const ProductSection: React.FC<ProductSectionProps> = ({ products, addToast }) => {
  ...
  const handleRegularPriceChange = (val: string) => {
    setRegularPrice(keepAndConvertEnglishDigits(val));
  };

  const handleDiscountPercentChange = (val: string) => {
    setDiscountPercent(keepAndConvertEnglishDigits(val));
  };
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!regularPrice || regPriceNum <= 0) return addToast("error", "Please enter a valid product price");

    const computedPrice = (discPercentNum > 0 && discPercentNum <= 100)
      ? Math.round(regPriceNum * (1 - discPercentNum / 100))
      : regPriceNum;
    ...
    try {
      ...
      // Process comma-separated custom sizes as defined in guidelines
      const processedCustomSizes = customSizesInput
        .split(',')
        .map(val => val.trim())
        .filter(val => val !== "");

      // Merge standard checkbox selections with parsed custom sizes
      const finalSizesToSave = [...selectedSizes, ...processedCustomSizes];

      await addDoc(collection(db, "products"), {
        name: name.trim(),
        price: computedPrice,
        originalPrice: discPercentNum > 0 ? regPriceNum : null,
        description: desc.trim(),
        category: category,
        sizes: finalSizesToSave,
        ...
        <div id="productSection" className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <form onSubmit={handleUpload} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase">Regular Price / আসল মূল্য (৳) *</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 rounded-xl pl-8 pr-3.5 py-2.5 text-sm outline-none transition font-mono font-bold text-slate-800"
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <label className="block text-xs font-bold text-slate-500 uppercase">Discount Percent / ছাড় (%)</label>
                  <span className="text-[10px] text-slate-400 font-bold lowercase italic font-sans pr-1">Optional</span>
                </div>
                <div className="relative">
                  <input
                    type="text"
                    ...
                    placeholder="যেমন: ১৫"
                    className="w-full border border-slate-200 focus:border-rose-500 focus:ring-1 focus:ring-rose-500/20 rounded-xl pl-3.5 pr-8 py-2.5 text-sm outline-none transition font-mono text-rose-500 font-bold"
                  />
                  <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-black">%</span>
                </div>
              </div>
            </div>

            {/* Dynamic computed sales price & strikethrough preview */}
            {regularPrice && (
              <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl text-xs flex flex-wrap items-center justify-between gap-2 shadow-2xs font-sans">
                <div className="flex items-center gap-2">
                  <span className="text-slate-400 text-[10px] font-extrabold uppercase">Calculated Sale Price:</span>
                  <span className="font-mono text-xs font-bold text-slate-800">
                    {Number(discountPercent) > 0 && Number(discountPercent) <= 100 ? (
                      <>
                        <span className="line-through text-slate-400 mr-2">৳{Number(regularPrice).toLocaleString()}</span>
                        <span className="text-emerald-600 font-extrabold text-sm">৳{Math.round(Number(regularPrice) * (1 - Number(discountPercent) / 100)).toLocaleString()}</span>
                      </>
                    ) : (
                      <span className="text-slate-900 font-extrabold text-sm">৳{Number(regularPrice).toLocaleString()}</span>
                    )}
                  </span>
                </div>
                {Number(discountPercent) > 0 && Number(discountPercent) <= 100 && (
                  <div className="bg-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full flex items-center gap-0.5 shadow-2xs animate-pulse">
                    ⚡ {discountPercent}% ছাড়
                  </div>
                )}
              </div>
            ...
            {/* Clothing Sizes Option */}
            {(category.includes("Fashion") || category.includes("Clothing") || category.includes("Dress")) && (
              <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-4 space-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Dress Sizes / পোশাকের সাইজ (Standard)</label>
                    ...
                    {["S", "M", "L", "XL", "XXL", "XXXL"].map((sz) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition duration-150 cursor-pointer flex items-center gap-1 ${
                            isSelected
                              ? "bg-slate-900 border-slate-900 text-white shadow-xs"
                              : "bg-white border-slate-200 text-slate-600 hover:border-slate-350"
                          }`}
                        >
                          {isSelected && <Check className="w-3.5 h-3.5 shrink-0 animate-scaleIn" />}
                          <span>{sz}</span>
                        </button>
                      );
                    })}
                ...
                <div className="pt-1.5 border-t border-slate-100">
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Custom Sizes / কাস্টম সাইজ (কমা দিয়ে লিখুন)</label>
                  <input
                    type="text"
                    value={customSizesInput}
                    onChange={(e) => setCustomSizesInput(e.target.value)}
                    placeholder="অন্যান্য সাইজ যেমন: ৩৯, ৪০, ৪১, ৪২ (কমা দিয়ে আলাদা করুন)"
                    className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold focus:outline-hidden focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 text-slate-800 transition"
                  />
                </div>
              </div>
            )}
