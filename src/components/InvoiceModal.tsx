        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          <div className="lg:col-span-5 no-print space-y-3.5 bg-slate-50/50 p-4 border border-slate-200/65 rounded-xl">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-0.5 uppercase">Courier Tracking ID</label>
                <div className="relative">
                  <ClipboardList className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-indigo-505" />
                  <input
                    type="text"
                    value={trackingNumber}
                    onChange={(e) => setTrackingNumber(e.target.value)}
                    placeholder="e.g. DEX-BDN-0109240258"
                    className="w-full pl-8.5 pr-3 py-1.5 text-xs border border-indigo-200 rounded-lg outline-none focus:border-indigo-500 focus:bg-white bg-indigo-50/10 tracking-wider transition font-mono font-bold text-slate-800"
                  />
                </div>
              </div>
          ...
          <div className="lg:col-span-7 flex flex-col items-center bg-slate-100/70 p-4 rounded-xl border border-slate-200/50 max-h-[520px] overflow-y-auto relative shadow-inner">
            ...
            <div
              ...
              className="border border-slate-200 p-6 rounded-lg bg-white shadow-md flex flex-col justify-between box-border text-slate-800 shrink-0 select-none overflow-hidden"
            >
              <div>
                {/* 1. Header Section */}
                <div className="flex justify-between items-start border-b border-slate-100 pb-2 mb-2">
                  <div>
                    <h1 className="text-base font-black tracking-wide text-slate-900 uppercase">SmartHaatBD</h1>
                    <p className="text-[8px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">Premium E-Commerce Platform</p>
                  </div>
                  <div className="text-right">
                    <span className="text-[9px] font-black text-white tracking-widest uppercase bg-slate-900 px-2 py-0.5 rounded">
                      Retail Invoice
                    </span>
                    <p className="text-[9px] font-mono text-slate-400 font-semibold mt-1">{formattedDate}</p>
                  </div>
                </div>

                {/* 2. Bill-To + Shipment info Grid */}
                <div className="grid grid-cols-12 gap-1 text-[11px] mb-2 pb-2 border-b border-dashed border-slate-200">
                  
                  {/* Bill to block */}
                  <div className="col-span-7 space-y-1">
                    <span className="text-slate-400 font-black uppercase text-[8px] block tracking-wide">Customer information:</span>
                    <p className="font-black text-slate-950 text-[13.5px] leading-tight select-all">{customerName || "No Name"}</p>
                    <p className="text-slate-950 font-mono font-bold text-[12px] bg-slate-100/85 px-1.5 py-0.5 rounded inline-block select-all mt-0.5">
                      📞 {customerPhone || "No Phone"}
                    </p>
                    <p className="text-slate-800 leading-snug text-[10px] font-bold pr-2 mt-1 max-h-[50px] overflow-hidden select-all">
                  ...
                  <div className="col-span-5 text-right space-y-1">
                    <div>
                      <span className="text-slate-404 text-slate-450 font-extrabold uppercase text-[7.5px] block tracking-wide">Order reference:</span>
                      <span className="font-mono text-[11.5px] text-emerald-900 font-black bg-emerald-50 border border-emerald-150 rounded px-1.5 py-0.2 inline-block">
                        {getOrderNumber(order)}
                      </span>
                    </div>

                    <div>
                      <span className="text-slate-450 font-extrabold uppercase text-[7.5px] block tracking-wide">Courier track ID:</span>
                      <span className="font-mono text-[10px] text-slate-800 font-bold bg-slate-50 border border-slate-200 rounded px-1.5 py-0.2 inline-block">
                        {finalTrackingNo}
                      </span>
                    </div>
                  </div>
                ...
                {order.paymentMethod?.toLowerCase() !== "cod" ? (
                  <div className="bg-emerald-50 border border-emerald-300 rounded-xl p-3 mb-2 text-center">
                    <span className="text-[8px] font-black text-emerald-850 uppercase tracking-widest block">Dispatch Directive</span>
                    <p className="text-sm font-black text-emerald-800 leading-none mt-0.5">PREPAID IN FULL (MFS)</p>
                    <div className="bg-emerald-600 text-white font-extrabold rounded-lg py-1 px-3 mt-1.5 text-[10.5px] max-w-[250px] mx-auto uppercase tracking-wider">
                      DO NOT COLLECT CASH FROM CUSTOMER
                    </div>
                  </div>
                ) : (
                  <div className="bg-rose-50 border border-rose-300 rounded-xl p-3 mb-2 text-center">
                    <span className="text-[8px] font-black text-rose-850 uppercase tracking-widest block">Dispatch Directive</span>
                    <p className="text-xs font-bold text-rose-700 leading-none mt-0.5">CASH ON DELIVERY (COD)</p>
                    <div className="bg-rose-600 text-white font-black rounded-lg py-1 px-4 mt-2 text-[13px] max-w-[250px] mx-auto uppercase tracking-wide shadow-xs">
                      Collect: ৳{totals.grandTotal.toLocaleString()} BDT
                    </div>
                  </div>
                )}

                {/* 4. Tracking Barcode representation */}
                <div className="flex flex-col items-center justify-center py-2.5 border-y border-dashed border-slate-200 my-2">
                  <img
                    src={`https://quickchart.io/barcode?type=code128&text=${encodeURIComponent(finalTrackingNo)}&width=240&height=36&showText=false`}
                    alt="Barcode"
                    referrerPolicy="no-referrer"
                    className="h-8 w-auto object-contain mix-blend-multiply"
                  />
                  <span className="font-mono text-[8.5px] font-bold text-slate-500 tracking-wider mt-0.5">*{finalTrackingNo}*</span>
                </div>

                {/* 5. Item list Table */}
                <table className="w-full text-left text-[11px] mb-2.5">
                  <thead>
                    <tr className="bg-slate-900 text-white uppercase text-[8px] tracking-wider font-extrabold">
                      <th className="p-1 px-2.5 rounded-l">Product Description</th>
                      <th className="p-1 text-center w-12">Qty</th>
                      <th className="p-1 text-right pr-2.5 rounded-r w-20">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-b border-slate-100">
                      <td className="p-1.5 px-2.5 font-bold text-slate-800 text-[10.5px]">
                        {order.productName || "E-Commerce Product"}
                      </td>
                      <td className="p-1.5 text-center text-slate-600 font-mono font-medium">1</td>
                      <td className="p-1.5 text-right pr-2.5 font-bold font-mono text-slate-800 text-[10.5px]">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-end">
                  ...
                  <div className="flex items-center gap-2">
                    <img
                      ...
                      alt="Invoice QR"
                      referrerPolicy="no-referrer"
                      className="w-11 h-11 object-contain rounded border border-slate-150 p-0.5 bg-white mix-blend-multiply"
                    />
                    <div>
                      <span className="text-[7.5px] font-black text-slate-450 uppercase tracking-widest block">Customer Copy</span>
                      <p className="text-[8px] text-slate-400 font-semibold leading-tight max-w-[110px]">
                        Scan to verify real-time status online.
                      </p>
                    </div>
                  </div>

                  {/* Calculations breakdown */}
                  <div className="w-52 text-[11px] space-y-1 font-bold text-slate-700">
                    <div className="flex justify-between">
                      <span>Subtotal:</span>
                      <span className="font-mono font-bold text-slate-900 text-[11px]">৳{totals.subtotal.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Courier charge:</span>
                      <span className="font-mono font-bold text-slate-900 text-[11px]">৳{totals.delivery.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between font-black text-xs text-slate-950 border-t border-slate-250 pt-1.5 bg-slate-50 px-2.5 py-1.5 rounded inline-flex w-full">
                      <span className="mr-auto text-slate-950 font-bold">Total Payable:</span>
                      <span className="text-slate-950 font-mono font-extrabold text-[13px]">৳{totals.grandTotal.toLocaleString()} BDT</span>
                    </div>
                  </div>
                ...
                <div className="border-t border-slate-100 pt-2 text-[8px] text-slate-400 text-center font-medium italic tracking-wide">
                  Thank you for shopping with SmartHaatBD! For query, check settings gateway.
                </div>
            ...
            <div className="no-print mt-3 flex items-center gap-1.5 text-slate-400 text-[10px] font-bold tracking-wider select-none uppercase">
              <span>✂️</span>
              <span>A5 Size Memo Cutting Border</span>
              <span>--------------------------------</span>
            </div>
