      ...
      <div onClick={onViewDetail} className="p-4 space-y-4 cursor-pointer flex-1">
        <div className="aspect-video relative overflow-hidden bg-slate-50 border border-slate-100 rounded-xl p-3 flex items-center justify-center">
          ...
          <div className="absolute top-2.5 right-2.5 flex flex-col items-end gap-1 z-10">
            <div className="bg-slate-900/90 text-white text-[10px] font-bold px-2.5 py-1 rounded-lg font-mono shadow-xs flex items-center gap-1.5 leading-none">
            ...
            {p.originalPrice && p.originalPrice > p.price && (
              <span className="bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md shadow-xs select-none leading-none animate-pulse">
              ...
              <div className="p-6 overflow-y-auto space-y-6 flex-1">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-left">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {detailProduct.originalPrice && detailProduct.originalPrice > detailProduct.price && (
                        <>
                          <span className="line-through text-slate-400 text-xs font-semibold">৳{detailProduct.originalPrice.toLocaleString()}</span>
                          <span className="bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md shadow-2xs select-none animate-pulse">
                            ⚡ {Math.round(((detailProduct.originalPrice - detailProduct.price) / detailProduct.originalPrice) * 100)}% ছাড়
                          </span>
                        </>
                      )}
                    </div>
                ...
                {detailProduct.sizes && detailProduct.sizes.length > 0 && (
                  <div className="space-y-2 text-left">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Available Sizes / সাইজসমূহ</h4>
                    <div className="flex flex-wrap gap-1.5 bg-slate-50 border border-slate-100 rounded-2xl p-4">
                      {detailProduct.sizes.map((sz) => (
                        <span key={sz} className="text-xs font-bold text-slate-700 bg-white border border-slate-200 shadow-3xs px-3 py-1 rounded-lg font-mono">
                          {sz}
                        </span>
                      ))}
                    </div>
